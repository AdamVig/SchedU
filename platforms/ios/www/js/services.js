...
// ScheduleDB Service //
////////////////////////
.service('ScheduleDB', function ($q) {

  var db = new PouchDB('https://schedu.iriscouch.com:6984/schedule');

  /**
   * Returns the schedule for today
   * @return {Object} ex: {"dayLetter":"A", dayNumber": 1}
   */
  this.getToday = function () {
    var today = moment().format("MM-DD-YY");
    var firstDay = moment('08/27/14', "MM/DD/YY");
    db.get(firstDay.format("MM/DD/YY")).then();
  }
...
.service('UserDB', function ($q) {
  ...
  /**
   * Remove user from user database by _id (phone number)
   * @param  {string} phoneNumber User's phone number, used as unique ID in database
   * @return {boolean}            True on success, false on failure
   * @TODO    Test and
   */
  this.deleteUser = function (phoneNumber) {

    db.get(phoneNumber).then(function(user) {
      return db.remove(user);
    }).catch(function(err){
      return "Could not delete user data. Please contact support.";
    });
  };
